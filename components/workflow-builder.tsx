...
import { ArrowLeft, Bot, CheckCircle2, Clock3, Database, Globe, Loader2, Mail, Pause, Play, Save, Webhook, Zap } from "lucide-react";
...
export function WorkflowBuilder({ existingWorkflow }: { existingWorkflow?: WorkflowRecord }) {
  ...
  async function ensureSaved(): Promise<string> {
    ...
    let id = workflowId;
    const response = await fetch(id ? `/api/workflows/${id}` : "/api/workflows", {
      method: id ? "PATCH" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, status: id ? status : "DRAFT", ...definition }),
    });
  ...
  async function saveWorkflow() {
    ...
    try { await ensureSaved(); setMessage("Workflow saved."); }
  ...
  async function toggleActive() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to change workflow status.");
      setStatus(data.workflow.status);
      setMessage(data.workflow.status === "ACTIVE" ? "Workflow activated." : "Workflow paused.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to change workflow status."); }
    finally { setBusy(null); }
  }
      ...
      <main className="main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="btn ghost" onClick={saveWorkflow} disabled={busy !== null}>{busy === "save" ? <Loader2 className="spin" size={15}/> : <Save size={16}/>} Save</button>
            <button className="btn ghost" onClick={toggleActive} disabled={busy !== null}>{busy === "status" ? <Loader2 className="spin" size={15}/> : status === "ACTIVE" ? <Pause size={15}/> : <Play size={15}/>} {status === "ACTIVE" ? "Pause" : "Activate"}</button>
            <button className="btn primary" onClick={testRun} disabled={busy !== null}>{busy === "run" ? <Loader2 className="spin" size={15}/> : <Play size={15}/>} Test run</button>
          </div>
        </header>
        {message && <div className="builder-toast">{message.includes("success") || message.includes("activated") ? <CheckCircle2 size={15}/> : null}{message}</div>}
        <div className="builder">
          <aside className="builder-sidebar">
            ...
            <div className="node-list">{palette.map(({ label, detail, type, icon: Icon }) => <button className="node-palette" key={label} onClick={() => setNodes((current) => [...current, { id: `${type}-${Date.now()}`, position: { x: 160 + current.length * 30, y: 320 + (current.length % 5) * 80 }, data: { label, type, config: type === "http" ? { method: "GET", url: "https://example.com" } : type === "schedule" ? { intervalMinutes: 5 } : {}, }, type: "default" }])}><span className="node-icon"><Icon size={15}/></span><span style={{textAlign:"left"}}><strong>{label}</strong><small>{detail}</small></span></button>)}</div>
            {workflowId && status === "ACTIVE" && <div className="card" style={{marginTop:18}}><div className="card-head"><h3>Webhook</h3><span className="badge">Live</span></div><p style={{color:"var(--muted)",fontSize:11,lineHeight:1.5}}>POST requests to this URL to trigger the active workflow.</p><code style={{display:"block",wordBreak:"break-all",fontSize:10,color:"#c7d0dd"}}>/api/webhooks/{workflowId}</code></div>}

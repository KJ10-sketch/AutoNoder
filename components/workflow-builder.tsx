...
import { ArrowLeft, Bot, CheckCircle2, Clock3, Database, Globe, KeyRound, Loader2, Mail, Pause, Play, Save, Settings2, Trash2, Webhook, Zap } from "lucide-react";
...
function fromDefinition(nodes: unknown, connections: unknown): { nodes: AppNode[]; edges: Edge[] } {
  ...
  return {
    nodes: workflowNodes.map((node: any, index) => ({ id: String(node.id ?? `node-${index}`), type: "default", position: node.position ?? { x: 120 + index * 250, y: 180 }, data: { label: String(node.name ?? node.type ?? "Node"), type: String(node.type ?? "webhook"), config: node.config ?? {} } })),
...
function CredentialSelect({ value, credentials, onChange }: { value: string; credentials: Credential[]; onChange: (value: string) => void }) {
  return <label className="field"><span>Credential</span><select value={value} onChange={(e) => onChange(e.target.value)}><option value="">Use environment API key / none</option>{credentials.map((credential) => <option key={credential.id} value={credential.id}>{credential.name} · {credential.provider}</option>)}</select></label>;
}
...
export function WorkflowBuilder({ existingWorkflow }: { existingWorkflow?: WorkflowRecord }) {
  ...
  useEffect(() => {
    fetch("/api/credentials").then(async (response) => response.ok ? (await response.json()).credentials as Credential[] : []).then(setCredentials).catch(() => setCredentials([]));
  }, []);
  ...
  async function ensureSaved(): Promise<string> {
    ...
    setWorkflowId(data.workflow.id); setStatus(data.workflow.status); return data.workflow.id;
  }
  async function saveWorkflow() { setBusy("save"); setMessage(""); try { await ensureSaved(); setMessage("Workflow saved successfully."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save workflow."); } finally { setBusy(null); } }
  async function testRun() {
    setBusy("run"); setMessage(""); setResult(null);
    try { const id = await ensureSaved(); const response = await fetch("/api/workflows/execute", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: id, input: { source: "manual-test", timestamp: new Date().toISOString() } }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Workflow execution failed."); setResult(data.context?.results ?? data.execution?.output ?? null); setMessage("Test run completed successfully."); } catch (error) { setMessage(error instanceof Error ? error.message : "Workflow execution failed."); } finally { setBusy(null); }
  }
  async function toggleActive() {
    setBusy("status"); setMessage("");
    try { const id = await ensureSaved(); const nextStatus = status === "ACTIVE" ? "PAUSED" : "ACTIVE"; const response = await fetch(`/api/workflows/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: nextStatus }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Unable to change workflow status."); setStatus(data.workflow.status); setMessage(data.workflow.status === "ACTIVE" ? "Workflow activated." : "Workflow paused."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to change workflow status."); } finally { setBusy(null); }
  }
  ...
  const needsCredential = selected?.data.type === "ai" || selected?.data.type === "http" || selected?.data.type === "email";

  return <div className="app-shell">
    <aside className="sidebar"><div className="brand">Auto<span>Noder</span></div><nav className="nav"><Link className="nav-item" href="/"><ArrowLeft size={17}/><span>Dashboard</span></Link><Link className="nav-item active" href="/workflows"><Zap size={17}/><span>Workflows</span></Link><Link className="nav-item" href="/executions"><Clock3 size={17}/><span>Executions</span></Link><Link className="nav-item" href="/settings/credentials"><KeyRound size={17}/><span>Credentials</span></Link></nav><div className="sidebar-footer">Build once. Execute reliably. Inspect every run.</div></aside>
    <main className="main"><header className="topbar"><div className="workflow-name-wrap"><input className="workflow-name" value={name} onChange={(e) => setName(e.target.value)} aria-label="Workflow name"/><small style={{color:"var(--muted)"}}>Visual automation builder</small></div><div className="topbar-actions"><span className="badge">{status}</span><button className="btn ghost" onClick={saveWorkflow} disabled={busy !== null}>{busy === "save" ? <Loader2 className="spin" size={15}/> : <Save size={16}/>} Save</button><button className="btn ghost" onClick={toggleActive} disabled={busy !== null}>{busy === "status" ? <Loader2 className="spin" size={15}/> : status === "ACTIVE" ? <Pause size={15}/> : <Play size={15}/>} {status === "ACTIVE" ? "Pause" : "Activate"}</button><button className="btn primary" onClick={testRun} disabled={busy !== null}>{busy === "run" ? <Loader2 className="spin" size={15}/> : <Play size={15}/>} Test run</button></div></header>
      ...
      <div className="builder"><aside className="builder-sidebar"><div className="builder-title"><h2>Add a node</h2><span className="badge">{palette.length} core nodes</span></div><input className="search" placeholder="Search nodes"/><div className="node-list">{palette.map(({ label, detail, type, icon: Icon }) => <button className="node-palette" key={label} onClick={() => addNode(type, label)}><span className="node-icon"><Icon size={15}/></span><span style={{textAlign:"left"}}><strong>{label}</strong><small>{detail}</small></span></button>)}</div>{workflowId && status === "ACTIVE" && <div className="card" style={{marginTop:18}}><div className="card-head"><h3>Webhook</h3><span className="badge">Live</span></div><p style={{color:"var(--muted)",fontSize:11,lineHeight:1.5}}>POST requests to this URL to trigger the active workflow.</p><code style={{display:"block",wordBreak:"break-all",fontSize:10,color:"#c7d0dd"}}>/api/webhooks/{workflowId}</code></div>}{result && <div className="run-result"><strong>Latest output</strong><pre>{JSON.stringify(result, null, 2)}</pre></div>}</aside>
        ...
        <aside className="inspector"><div className="builder-title"><h2>Node settings</h2><Settings2 size={17}/></div>{!selected && <div className="empty-inspector"><Settings2 size={28}/><p>Select a node on the canvas to edit its configuration.</p></div>}{selected && <div className="inspector-form"><Field label="Node name" value={selected.data.label} onChange={(value) => patchSelected({label:value})}/><div className="node-type">Type: <strong>{selected.data.type}</strong></div>{needsCredential && <CredentialSelect value={String(selected.data.config.credentialId ?? "")} credentials={credentials} onChange={(value) => updateConfig("credentialId", value || undefined)}/>} 
